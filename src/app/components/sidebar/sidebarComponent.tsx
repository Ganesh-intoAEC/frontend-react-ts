...
export default function MiniDrawer() {
  ...
  const [activeLink, setActiveLink] = React.useState<string>(''); // Initialize activeLink state

  const handleLinkClick = (to: string) => {
    setActiveLink(to); // Set the active link when a link is clicked
  };
          ...
          {[
            ...
            <Tooltip title={item.text} arrow placement="right" key={item.text}>
              <Link
                ...
                style={{
                  textDecoration: 'none',
                  color: item.to === activeLink ? '#3CA2FF' : '#334D6E',
                }}
              >
